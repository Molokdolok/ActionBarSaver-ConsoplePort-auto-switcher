...
frame:SetScript("OnEvent", function(self, event, addon)
	if( event == "ADDON_LOADED" and addon == "ActionBarSaver" ) then
		...
		-- Check if we should auto-load on login
		if( IsLoggedIn() ) then
			if (ABS.db.autoSwitch) then
				AutoLoad(IsAddOnLoaded("ConsolePort") and "ConsolePortON" or "ConsolePortOFF")
			end
		else
	...
	elseif( event == "PLAYER_LOGOUT" ) then
		if( ABS.db.lastProfile ) then
			ABS:SaveProfile(ABS.db.lastProfile)
		end
	end
end)